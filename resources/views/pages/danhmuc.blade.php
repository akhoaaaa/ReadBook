@extends('layout')
@section('content')
<section class="product spad">
    <div class="container">
        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{url('/')}}">Trang chủ</a></li>
                <li class="breadcrumb-item"><a href="{{url('danh-muc',[$danhmuctruyen->slug])}}">{{$danhmuctruyen->tendanhmuc}}</a></li>
            </ol>
        </nav>

        <div class="row">
            <div class="col-lg-3 col-md-5">
                <div class="sidebar">
                    <div class="sidebar__item">
                        <h4>Thể loại</h4>
                        <ul>
                            @foreach($theloai as $key => $cate)
                            <li><a href="{{route('the-loai',[$cate->slug])}}">{{$cate->tentheloai}}</a></li>
                            @endforeach
                        </ul>
                    </div>
                    <div class="sidebar__item">
                        <div class="latest-product__text">
                            <h4>Truyện mới nhất</h4>
                            <div class="latest-product__slider owl-carousel">
                            @foreach($truyenmoi as $key => $trmoi)

                                <div class="latest-prdouct__slider__item">
                                    <a href="{{route('truyen-tranh',[$trmoi->slug_truyen])}}" class="latest-product__item">
                                        <div class="latest-product__item__pic">
                                            <img src="{{asset('public/uploads/images/'.$trmoi->hinhanh)}}" alt="">
                                        </div>
                                        <div class="latest-product__item__text">
                                            <h6>{{$trmoi->tentruyen}}</h6>
                                            <span>{{$trmoi->gia}}</span>
                                        </div>
                                    </a>
                                </div>
                                @endforeach

                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-9 col-md-7">
                <div class="filter__item">
                    <div class="row">
                        <div class="col-lg-4 col-md-5">
                            <div class="filter__sort">
                                <span>Sắp xếp</span>
                                <select>
                                    <option value="0">Default</option>
                                    <option value="0">Default</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-lg-4 col-md-4">
                            <div class="filter__found">
                                <h6><span>100</span> Truyeện</h6>
                            </div>
                        </div>
                        <div class="col-lg-4 col-md-3">
                            <div class="filter__option">
                                <span class="icon_grid-2x2"></span>
                                <span class="icon_ul"></span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="row">
                    @php
                    $count = count($truyen);
                    @endphp
                    @if($count == 0)
                    <div class="col-lg-4 col-md-6 col-sm-6">

                        <p>Truyện đang cập nhật....</p>
                    </div>
                    @else
                    @foreach($truyen as $key => $tr)
                    <div class="col-lg-4 col-md-6 col-sm-6">
                        <div class="product__item">
                            <div class="product__item__pic set-bg" data-setbg="{{asset('public/uploads/images/'.$tr->hinhanh)}}">
                                <ul class="product__item__pic__hover">
                                    <li><a href="#"><i class="fa fa-heart"></i></a></li>
                                    <li><a href="#"><i class="fa fa-retweet"></i></a></li>
                                    <li><a href="#"><i class="fa fa-shopping-cart"></i></a></li>
                                </ul>
                            </div>
                            <div class="product__item__text">
                                <h6><a href="{{route('truyen-tranh',[$tr->slug_truyen])}}">{{$tr->tentruyen}}</a></h6>
                                <h5><i class="fa fa-eye"></i> 15000</h5>
                            </div>
                        </div>
                    </div>
                    @endforeach
                    @endif

                </div>
                <div class="product__pagination">
                    <a href="#">1</a>
                    <a href="#">2</a>
                    <a href="#">3</a>
                    <a href="#"><i class="fa fa-long-arrow-right"></i></a>
                </div>
            </div>
        </div>
    </div>
</section>
@endsection